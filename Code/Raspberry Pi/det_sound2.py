...
# num to text
def ntt(num, class_name):
    if num == 1:
        return "There is 1 person detected" + class_name
    else:
        return "There are " + str(num) + " people detected " + class_name

def ann(num_drowsy):
    drowsy_text = ntt(num_drowsy, "Drowsy")
    
    engine.stop()
    
    engine.say(drowsy_text)
    engine.runAndWait()
...
while True:
    ...
    if current_time - last_frame_time >= frame_interval:
        # Reset array untuk setiap frame
        last_predicted_array = []
        ...
        num_drowsy = last_predicted_array.count("Drowsy")
