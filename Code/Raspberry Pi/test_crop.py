...
def save_image(image, output_dir, prefix):
    ...
    num_files = len(files)
    filename = f"{prefix}_{num_files}.png"
...
while True:
    ...
    if current_time - last_frame_time >= frame_interval:
        ...
        for (x, y, w, h) in faces:
            ...
            save_image(crop_img, "hasil_deteksi/face", "face")
            ...
            crop_img = resize_image(crop_img, 640, 640)
            ...
            save_image(crop_img, "hasil_deteksi/landmark", "landmark")
            ...
            if name_classes[predicted_class] == "Drowsy":
                drowsy_count += 1
                save_image(crop_img, "hasil_deteksi/classify/Drowsy", "Drowsy")  # Save entire processed frame in "Drowsy" folder
            if name_classes[predicted_class] == "Neutral":
                neu_count += 1
                save_image(crop_img, "hasil_deteksi/classify/Neutral", "Neutral")  # Save entire processed frame in "Neutral" folder
    ...
    if elapsed_time >= TIME_INT:
        ann = f"Students detected Drowsy {drowsy_count} times in the past 10 minutes."
        print(ann)
